# yyyyMMdd - BUREAU EMAIL VIA - TALENTOS.txt
# Este script move as cargas de enriquecimento de e-mail da Via Varejo, bases 18 e 55, para os diretórios de importação do CRM.
# This script moves the Via Varejo email enrichment loads, bases 18 and 55, to the CRM import directories.
...
# Remote Paths
$remotePath = [System.Environment]::GetEnvironmentVariable("REMOTE_PATH_BE")
$remoteProcessedPath = [System.Environment]::GetEnvironmentVariable("REMOTE_PROCESSED_PATH_BE")

# Get today date yyyyMMdd
$currentDateStr = (Get-Date).ToString("yyyyMMdd")

# File Name
$fileNamePattern = "$currentDateStr - BUREAU EMAIL VIA - TALENTOS.txt"
...
if ($errorLog -ne "") {
    ...
    $mailMessage.Subject = "ERRO NO BUREAU DE EMAILS"
...
if ($successLog -ne "" -and $errorLog -eq "") {
    ...
    $mailMessage.From = $userName
    $mailMessage.To.Add($to)
    $mailMessage.Subject = "BUREAUS DE EMAILS PROCESSADO COM SUCESSO!"
